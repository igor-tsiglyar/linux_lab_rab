...
#define NOP  8                                    /*размер табл.операций    */
   ...
   {                                              /*то:                     */
    if                                            /* если псевдооперация DC */
     (                                            /* определяет константу   */
      TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='F'/* типа F или             */
   || strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,/* типа B длиной 4 байта */
              "BL4", 3) == 0
                                                  /* то выполнить   */
     )                                            /* следующее:             */
     {
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  else if (strncmp (TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,
                    "BL4", 3) == 0)               /* если операнд начинается*/
                                                  /* с комбинации BL4       */
   {
    RAB=strtok ((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND + 4, "'");
    RX.OP_RX.R1X2 = 0;                            /*занулим три младших     */
    RX.OP_RX.B2D2 = 0;                            /*байта RX.OP_RX          */
    char value_s[8];
    strcpy(value_s, RAB);
    memset(value_s + strlen(RAB), '0', 8 - strlen(RAB));
    RX.OP_RX.OP = strtol(value_s, NULL, 2);       /*перевод ASCII -> int    */
   }
  else                                            /*иначе                   */
...
int SUSING()                                      /*подпр.обр.пс.опер.USING */
 {
  ...
  if ( *METKA2 == '@' )                           /*если лексема начинается */
   {                                              /*с @, то:            */
...
int SRR()                                         /*подпр.обр.опер.RR-форм. */
 {
  ...
  if ( *METKA1 == '@' )                           /*если лексема начинается */
   {                                              /*с @, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *METKA1 */
